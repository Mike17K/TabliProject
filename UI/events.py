import pygame
from UI.constants import screen_height, screen_width, whitePiece, blackPiece, PIECE_WIDTH, SECTION_TOP_LEFT, SECTION_TOP_RIGHT, OFFSETS
...
def handleEvents(event,state):
    if event.type == pygame.MOUSEBUTTONDOWN:
        ...
        if state.holding_piece is None and clicked_index in [24,25] and (1 in state.board.captured_pieces if clicked_index == 24 else -1 in state.board.captured_pieces):
            state.holding_piece = 1 if clicked_index == 24 else -1
            state.holding_piece_previews_index = clicked_index
        if clicked_index is not None and 0<= clicked_index <24 and state.board.board[clicked_index] != 0:
            state.holding_piece = 1 if state.board.board[clicked_index] > 0 else -1
            state.holding_piece_previews_index = clicked_index
    if event.type == pygame.MOUSEBUTTONUP:
        if state.holding_piece is not None:
            clicked_index = get_board_index(*pygame.mouse.get_pos())
            state.board = state.board.move(state.holding_piece_previews_index,clicked_index)
