...
class Board: 
    ...
    def GetAvailableMovesFromDice(self,dice: int,is_white: bool) -> list[int,int]: 
        ...
        for i in range(24-dice):
            ...
            if self.board[index] == 0 or self.board[index] * (1 if is_white else -1) < 0: continue
            if self.board[next_index] in [-1,1,0] or self.board[next_index] * (1 if is_white else -1) > 0:
    ...
    def CostOfMove(self,from_index,to_index) -> float:
        ...
        score = 0
        for i in range(24):
            if newBoard.board[i] == 0: continue
            # score += newBoard.board[i] * (i-(22 if newBoard.board[i] > 0 else 2))
            if newBoard.board[i] == 1: score += -10
            if newBoard.board[i] == -1: score += 10
    ...
    def GetBestMoveForDices(self,dice1: int,dice2: int,is_white: bool) -> list[int,int]:
        moves1 = self.GetAvailableMovesFromDice(dice1,is_white)
        ...
        for m1 in moves1:
            ...
            for m2 in moves2:
                ...
                if best_cost==None or cost * (1 if is_white else -1) >= best_cost * (1 if is_white else -1):
                    best_cost = cost
                    best_move1 = m1
                    best_move2 = m2
                    print(best_cost)
        return best_move1,best_move2
...
class UIState:
    ...
    @staticmethod
    def get_instance():
        if UIState.INSTANCE is None: UIState.INSTANCE = UIState()
