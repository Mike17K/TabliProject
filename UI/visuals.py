...
def DrawState(state):
    ...
    screen.blit(
        pygame.transform.scale(background_image, (screen_width, screen_height)), (0, 0)
    )
    ...
    for i in range(len(tmp)):
        if tmp[i] > 0:
            screen.blit(
                pygame.transform.scale(whitePiece, (PIECE_WIDTH, PIECE_WIDTH)),
                [
            ...
            screen.blit(
                pygame.transform.scale(blackPiece, (PIECE_WIDTH, PIECE_WIDTH)),
                [
                    screen_width / 2 - PIECE_WIDTH / 2,
                    screen_height / 2
